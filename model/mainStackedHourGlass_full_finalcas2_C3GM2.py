...
import model.gm_twogate_ag as M
...
class Gmscenet(nn.Module):
    ...
    def __init__(self, nChannels, nStack, nModules, numReductions, nJoints):
        ...
        self.start = M.ConvBnRelu(3, 64, kernelSize=7, stride=2, padding=3)
        ...
        for i in range(self.nStack):
            ...
            if (i == self.nStack - 1):
                ...
                for j in range(self.nJoints):
                    ...
                    # feavis = FeatureVisualization(p[j], j)  # visualize enhanced structure-context features for each point
                    # feavis.save_feature_to_img()
                    kout[j] = self.chantojoints[j](p[j])

                    # feavis = FeatureVisualization(kout[j], j)  # visualize enhanced structure-context features for each point
                    # feavis.save_feature_to_img()
